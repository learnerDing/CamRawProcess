...
float* Raw2RGBEntry(unsigned char* rawdata,int rows ,int cols)
{	
	int In_width = cols;
	int In_height = rows;
	...
	out_height += pad_w;
	RawProcessor scaler;
	unsigned char* dw_img4 = new unsigned char[out_width*out_height*3];//接收Bayer转RGB的三通道图像
	...
	//注意：这里输出的dw_img5图像是HWC排列，每个像素颜色顺序是BGR
	Crop_Area_Image(dw_img4, dw_img5, out_width, pad, pad, out_width - pad_w, out_height - pad_w);//裁剪掉之前加上的边框存在dw_img5

	out_width -= pad_w;
	out_height -= pad_w;
	// PrintValues(dw_img5,4021148,4021248);
	float* OutImg = convertUnsignedCharToFloat(dw_img5,3,out_width,out_height);
	return OutImg;
	delete dw_img4;
	delete dw_img5;
	delete pad_img;
