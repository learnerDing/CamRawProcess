...
__global__ void GammaCorrectionKernel(T* img_data, const T* lut, int rows, int cols) {
    ...
    // 使用 LUT 进行 Gamma 矫正
    img_data[r_idx] = lut[static_cast<int>(r)];
    img_data[g_idx] = lut[static_cast<int>(g)];
    img_data[b_idx] = lut[static_cast<int>(b)];
}
...
template void launchGammaCorrection(TensorWrapper<float>* d_img_tensor, TensorWrapper<float>* d_lut_tensor, int rows, int cols);